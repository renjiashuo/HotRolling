...
#pragma region Variables	
	...
	map<pair<pair<pair<pair<pair<double,double>,double>,double>,double>,double>, TortoiseShell*> temp;			// 存放放钢卷组过程中更好的kpi及各分量、乌龟壳
		...
		for (Another_Shell = Chosen_Shell + 1; Another_Shell <= s_mapSetOfTortoiseShell.size(); Another_Shell++)
		{
			...
			for (; iter != An_groups_backups.end(); iter++)
			{
				...
				for (; iter1 != groups_backups.end(); iter1++)
				{
					...
					if (iter1 == groups_backups.begin())
					{
						...
						else
						{
							...
							temp.insert(make_pair(make_pair(make_pair(make_pair(make_pair(make_pair(temp_kpi, flow_rate), order_rate), rollingkm_rate), DHCR_rate), Scheduling_quality), tortoiseShell));
							tortoiseShell->m_main_groups.clear();
							tortoiseShell->m_main_groups = groups_backups;
							// 重新放入垃圾桶
				...
				if (!groupInTurn.empty())
				{
					...
					for (map<pair<pair<pair<pair<pair<double, double>, double>, double>, double>, double>, TortoiseShell*>::iterator iter7 = temp.begin(); iter7 != temp.end(); iter7++)
					{
						if (iter7->first.first.first.first.first.first > best_kpi)
						{
							best_kpi = iter7->first.first.first.first.first.first;
							cout << "流向匹配率: " << iter7->first.first.first.first.first.second << "   " << "合同计划兑现率: " << iter7->first.first.first.first.second << "    " << "轧制公里率: " << iter7->first.first.first.second << "   " << "DHCR比率: " << iter7->first.first.second << "   " << "排程质量: " << iter7->first.second << endl;
